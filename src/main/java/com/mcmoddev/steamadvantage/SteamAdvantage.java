...
import com.mcmoddev.steamadvantage.init.*;
		...
		for(String e : strs){
			if(e.contains("=")){
				...
				try{
					...
					LOGGER.error("Fluid fuel descriptor '%s' is not valid!",e);
				}
				LOGGER.info("%s: adding fluid fuel override for fluid '%s' with a burn time of %s game ticks per bucket",MODID,fluidName,value);
			} else {
				LOGGER.error("Fluid fuel descriptor '%s' is not valid!",e);
			}
		}
