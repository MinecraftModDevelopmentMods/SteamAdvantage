 ...
 *
 */
public class SteamPumpBlock extends com.mcmoddev.poweradvantage.api.simple.BlockSimplePowerMachine{
	...
	private void destroyPipe(World w, BlockPos coord) {
		...
		while(c.getY() > 0 && w.getBlockState(c).getBlock() == Materials.DEFAULT.getBlock(SteamAdvantageNames.PUMP_PIPE_STEAM)){
			w.setBlockToAir(c);
			c = c.down();
		}
	}
