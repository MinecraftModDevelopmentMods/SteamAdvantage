...
public class SteamPumpTileEntity extends com.mcmoddev.poweradvantage.api.simple.TileEntitySimplePowerMachine implements IFluidHandler {
	...
	public void tickUpdate(boolean isServerWorld) {
		if(isServerWorld){
			Block pump_pipe = Materials.DEFAULT.getBlock(SteamAdvantageNames.PUMP_PIPE_STEAM);
			if(!redstone){
				...
				if(timeUntilNextPump == 0 && getTank().getFluidAmount() <= 0){
					...
					while(target.getY() > 0 && w.getBlockState(target).getBlock() == pump_pipe){
						target = target.down();
					}
					if(target.getY() > 0 && w.isAirBlock(target) && getEnergy(Power.steam_power) >= ENERGY_COST_PIPE){
						// place pipe
						w.setBlockState(target, pump_pipe.getDefaultState());
	...
	public boolean isPowerSource(ConduitType type){
		return com.mcmoddev.poweradvantage.init.Fluids.isFluidType(type);
	}
	/**
